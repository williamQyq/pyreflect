...
125k curves - Best Trial: 6 layers, 0.0873 dropout, batch 64
"""

import modal

# Best parameters from 125k Optuna search
LAYERS = 6
DROPOUT = 0.0872577142694922
BATCH_SIZE = 64
EPOCHS = 20

app = modal.App("pyreflect-train-final-125k")
...
image = (
    ...
    .add_local_dir("../125k_curves", remote_path="/root/data", copy=True)
)


@app.function(gpu="T4", image=image, secrets=[modal.Secret.from_name("wandb-secret")], timeout=4*60*60)
def train_final_model():
    ...
    wandb.init(
        ...
        name=f"FINAL_125k_{LAYERS}L_{DROPOUT:.4f}D_{EPOCHS}E",
        config={"layers": LAYERS, "dropout": DROPOUT, "batch_size": BATCH_SIZE, "epochs": EPOCHS, "training_curves": 125000}
    )
...
def main():
    print("🚀 Training 125k final model on Modal GPU...\n")
    model_bytes = train_final_model.remote()

    from pathlib import Path
    output = Path("best_model_125k/trained_model.pth")
